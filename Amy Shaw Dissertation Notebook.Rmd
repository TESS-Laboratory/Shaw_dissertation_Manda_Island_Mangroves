...
The MICP and Lamu County data frames are first merged to create a new data frame. The year 1996 is then filtered out as it does not have a data point to compare to. A line graph is then created to show the percentage changes in mangrove cover between each obseration year for both Lamu County and the MICP. Lastly, the graph is saved to the plots folder.
...
Create scatter plots and bar charts to visualise the absolute change in mangrove cover in the MICP and Lamu County across the 24 year period of observation. Bar chart Y axes are not truncated to provide context as to the relative change over time. The line graph Y axis' are scaled at 3% above the maximum value and 3% below the minimum value for both MICP and Lamu County to minimise graph distortion. Summaries of each data frame are created to check the max and min values. Scatter plots and bar charts are created in one image and saved into the plots folder.
...
Define the MICP line graph Y axis limit as 3% above the MICP data Max value to normalise the data.
...
Define the MICP line graph Y axis limit as 3% below the MICP data Min value to normalise the data.
...
Define the Lamu line graph Y axis limit as 3% above the Lamu data Max value to normalise the data.
...
Define the Lamu line graph Y axis limit as 3% below the Lamu data Min value to normalise the data.
