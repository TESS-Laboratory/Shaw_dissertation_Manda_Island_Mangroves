...
The Lamu County data for each observation year is merged into a single data frame. The data is then aggregated to return the total Mangrove coverage in each year and converted into a new data frame. Next, the Total_Mangroves_Hectares column is created, which converts the Mangrove coverage figures from square meters to hectares. Lastly, the Total_Mangroves column, measured in square meters is dropped.
...
Aggregate the data to return the total mangrove coverage in square meters by year.
...
Convert aggregated data into a new dataframe
...
Drop Total_Mangroves (sq meters) column as unnecessary.
...
The MICP data for each observation year is merged into a single dataframe. The data is then aggregated to return the total Mangrove coverage in each year. Next, the Total_Mangroves_Hectares column is created, which converts the Mangrove coverage figures from square meters to hectares. Lastly, the Total_Mangroves column, measured in square meters is dropped.
...
#### Convert mangrove cover to hectares as in sq meters
...
#### Drop Total_Mangroves (sq meters) columns as unnecessary
...
#### Calculate percentage change between the MICP years of data.

##### The % change calculated is the difference from the previous year
...
##### Round up percentage change numbers
...
##### Create location column called MICP
...
#### Calculate percentage change between the Lamu County years of data.
...
##### Create location column called Lamu County
...
##### This is done because 1996 doesn't have a previous year to compare it to
...
##### The graph highlights the differences between MICP and Lamu County
...
#### Create scatterplots and bar charts to show the difference between the areas

##### Convert Manda year column to numeric

```{r}
df7$year = as.numeric(as.character(df8$year)) 
df7
```
...
Create time series Line plots showing mangrove coverage over time in Lamu County and the MICP
...
Create bar plots showing mangrove coverage over time in Lamu County and the MICP
...
fig3 <- p3 + p4 + plot_annotation(tag_levels = 'a') + 
  plot_annotation(title = 'Lamu County and MICP mangrove cover 1996 to 2020',
...
```{r}
filename = "Lamu County vs MICP cover change 1996-2020 Bar Chart"
